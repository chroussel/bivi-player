...
use super::element_id::{element_id_to_type, id_to_element_id};
use super::{
...
pub trait ParsableElement<R: Read + Seek> {
    type Output;
...
pub fn expect_master<R: Read + Seek>(
    r: &mut R,
...
pub fn collect_children<R: Read + Seek>(
    r: &mut R,
    offset: u64,
    size: u64,
...
pub fn parse_children_at_offset<R, T>(
    r: &mut R,
    offset: u64,
    master_id: ElementId,
    child_id: ElementId,
...
pub fn try_find_custom_type_or<T: From<u64>>(
    fields: &[(ElementId, ElementData)],
    element_id: ElementId,
    default: T,
) -> Result<T> {
...
pub fn try_find_custom_type<T: From<u64>>(
    fields: &[(ElementId, ElementData)],
    element_id: ElementId,
) -> Result<Option<T>> {
    let value = match try_find_unsigned(fields, element_id)? {
...
pub fn find_custom_type<T: From<u64>>(
    fields: &[(ElementId, ElementData)],
    element_id: ElementId,
) -> Result<T> {
    let value =
...
pub fn try_find_binary<R: Read + Seek>(
    r: &mut R,
    fields: &[(ElementId, ElementData)],
    element_id: ElementId,
...
pub fn next_element<R: Read + Seek>(r: &mut R) -> Result<(ElementId, ElementData)> {
...
pub fn parse_element_header<R: Read + Seek>(
    r: &mut R,
    from: Option<u64>,
...
pub fn parse_variable_i64<R: Read>(r: &mut R) -> Result<i64> {
    loop {
        let mut bytes = [0u8];
        r.read_exact(&mut bytes)?;
        let element_id = match bytes[0] {
